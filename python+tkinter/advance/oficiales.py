...
class Persona:
    ...
    def __init__(self, nombre=NONE, fecha=NONE, raza=NONE, rango=NONE, correo=NONE, sector=NONE):
        self.nombre = nombre
        self.raza  = raza
        self.fecha = fecha
        self.rango = rango
    ...
    def __str__(self):
        return 'Nombre: {0}\nRaza: {1}\nFecha: {2}\nRango: {3}\nCorreo: {4}\nSector: {5}'.format(self.nombre, self.raza, self.fecha, self.rango, self.correo, self.sector)
    ...
    def cargadatos(self):
        lista = []
        datos = [self.nombre, self.raza, self.fecha, self.rango, self.correo,self.sector]
        for dato in range(len(datos)):
            if datos[dato] != 'none':                
                lista.append(datos[dato])
        return tuple(lista)